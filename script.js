...
,                 
        ...
        const themes = {
            ...
            'alam': 'theme-alam.css',
            'angkasa': 'theme-angkasa.css',
            'salju': 'theme-snow.css',
            'sunset': 'theme-sunset.css',
            'noir': 'theme-spidermanNoir.css'
        };
    ...
    // Reset semua badge dulu
            document.querySelectorAll('.theme-badge').forEach(badge => {
                ...
                badge.style.background = 'var(--badge-inactive-bg, #ccc)';
                badge.style.color = 'var(--badge-inactive-text, #666)';
            });
        
            // Highlight tema aktif pakai variabel tema
            const activeBadge = document.getElementById(`badge-${currentTheme}`);
            if (activeBadge) {
                activeBadge.textContent = '✓ Aktif';
                activeBadge.style.background = 'var(--badge-active-bg, #4caf50)';
                activeBadge.style.color = 'var(--badge-active-text, white)';
            }
        }
        ...
        // 🎯 GENERATE HTML THEME OPTIONS (OTOMATIS)
        function generateThemeOptions() {
            ...
            updateThemeBadges(); // Sync badge
        }
        
        // 🧱 1 BARIS HTML THEME OPTION
        function createThemeOption(key, path) {
            ...
            div.innerHTML = `
                <div class="theme-preview ${key}-theme">
                ...
                <div class="theme-info">
                    ...
                    <span class="theme-badge" id="badge-${key}">Pilih</span>
                </div>
            `;
            return div;
        }
        
        // 🎨 EMOJI & DESKRIPSI (bisa di-edit)
        function getThemeEmoji(key) {
            const map = {
                normal: '🏪', estetik: '🌸', elegan: '💼',
                cerah: '☀️', gelap: '🌙', alam: '🌿', angkasa: '🚀', salju: '❄️', sunset: '🌅',
                noir: '🕷️'
            };
            return map[key] || '🎨';
        }
        function getThemeDesc(key) {
            const map = {
                ...
                estetik: 'Warna pastel yang lembut',
                elegan: 'Dark mode premium',
                gelap: 'Tema gelap & minimalis',
                alam: 'Tema alam & natural',
                angkasa: 'Tema futuristik & modern',
                salju: 'Tema dingin & segar',
                sunset: 'Tema hangat & tenang',
                noir: 'Tema spider man noir hitam putih'
            };
